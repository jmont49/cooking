...
1. In Shua Settings, create a one-time Miso token with only `inventory:read`, `inventory:write`, `recipes:read`, and `recipes:write`.
...
## Shua recipe requests handled by Miso
...
If Miso or the worker is offline, new requests remain queued. Miso never receives a Supabase service-role key or the Unsplash key. Unsplash search happens inside the Supabase function after the recipe passes validation.
