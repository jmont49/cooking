...
export const generatedRecipeSchema = z.object({ title: z.string().min(3), description: z.string(), servings: z.number().int().positive(), prepMinutes: z.number().int().nonnegative(), cookMinutes: z.number().int().positive(), difficulty: z.enum(['Easy', 'Medium', 'Ambitious']), estimatedCost: quantitySchema, ingredients: z.array(ingredientAmountSchema).min(1), steps: z.array(z.string().min(3)).min(1), equipment: z.array(z.string()), safetyNotes: z.array(z.string()), substitutions: z.array(z.string()), tags: z.array(z.string()), cuisine: z.string(), flavorProfile: z.array(z.string()), leftoverGuidance: z.string(), confidence: z.number().min(0).max(1), unresolvedIngredientMappings: z.array(z.string()) });
