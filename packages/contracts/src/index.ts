...
export const generatedRecipeSchema = z.object({ title: z.string().min(3), description: z.string(), servings: z.number().int().positive().max(24), prepMinutes: z.number().int().nonnegative().max(480), cookMinutes: z.number().int().positive().max(720), difficulty: z.enum(['Easy', 'Medium', 'Ambitious']), estimatedCost: quantitySchema, ingredients: z.array(ingredientAmountSchema).min(1).max(40), steps: z.array(z.string().min(3)).min(1).max(30), equipment: z.array(z.string()).max(20), safetyNotes: z.array(z.string()).max(20), substitutions: z.array(z.string()).max(20), tags: z.array(z.string()).max(20), cuisine: z.string().min(1), protein: z.string().min(1), cleanup: z.number().int().min(1).max(5), leftoverQuality: z.number().int().min(1).max(5), leftoverDays: z.number().int().min(0).max(7), reheating: z.string(), flavorProfile: z.array(z.string()), leftoverGuidance: z.string(), imageSearchQuery: z.string().min(3).max(160), confidence: z.number().min(0).max(1), unresolvedIngredientMappings: z.array(z.string()) });
