...
import { budgetSummary, consume, convert, createDemoWeeklyPrepPlan, formatQuantity, groceryList, projected, reserve, scaleRecipe, scoreRecommendation, validatePrepPlanQuantities } from './index.js';
...
describe('deterministic domain', () => {
  ...
  it('treats listed kitchen ingredients as sufficiently stocked', () => { expect(groceryList([{ recipe, servings: 6 }], [chicken])).toEqual([]); });
  it('scales servings and calories while keeping piece proteins whole',()=>{const pieceRecipe={...recipe,protein:'Chicken breast',ingredients:[{ingredientId:'chicken',name:'Chicken breast',quantity:'2',unit:'count' as const}]};expect(scaleRecipe(pieceRecipe,3,600).ingredients[0]!.quantity).toBe('3');expect(scaleRecipe(pieceRecipe,3,1000).ingredients[0]!.quantity).toBe('6');});
  it('formats ordinary cooking quantities as fractions',()=>{expect(formatQuantity('0.75')).toBe('3/4');expect(formatQuantity('1.5')).toBe('1 1/2');});
