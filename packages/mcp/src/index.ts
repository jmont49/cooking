...
const baseUrl=(process.env.MISE_API_URL??'http://127.0.0.1:54321/functions/v1/api/api/v1').replace(/\/$/,'');
const token=process.env.MISE_INTEGRATION_TOKEN;
if(!token){process.stderr.write('MISE_INTEGRATION_TOKEN is required\n');process.exit(1)}
...
const server=new McpServer({name:'mise-cooking-tools',version:'1.0.0'});
