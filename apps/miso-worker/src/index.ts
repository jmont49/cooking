...
interface Job{id:string;request:{protein:string;minutes:number;servings:number;notes:string;preferences?:{preferredProteins?:string[];exclusions?:string[];tasteProfile?:string};avoidRecipeTitles?:string[]}}
...
async function askMiso(job:Job,catalog:Ingredient[],inventory:InventoryRow[]):Promise<GeneratedRecipe>{
  const schema={title:'unique string unlike every avoidRecipeTitles entry',description:'string',servings:'integer exactly equal to request.servings',prepMinutes:'integer',cookMinutes:'integer',difficulty:'Easy | Medium | Ambitious',estimatedCost:'decimal string for the entire recipe',caloriesPerServing:'integer estimate 50-3000',proteinGramsPerServing:'integer estimate 0-300',cuisine:'string',protein:'string',cleanup:'integer 1-5',leftoverQuality:'integer 1-5',leftoverDays:'integer exactly 1',reheating:'string for lunch tomorrow',ingredients:[{ingredientId:'canonical UUID when matched; otherwise unresolved',name:'string',quantity:'positive decimal string',unit:'one of count, serving, tsp, tbsp, fl_oz, cup, oz, lb, g, kg, ml, l, package, bag, can, bottle, bunch',optional:'boolean'}],steps:['string'],equipment:['string'],safetyNotes:['string including allergy cross-contact guidance when relevant'],substitutions:['string'],tags:['string'],flavorProfile:['string'],leftoverGuidance:'string for exactly one day of leftovers',imageSearchQuery:'must be exactly identical to title',confidence:'number 0-1',unresolvedIngredientMappings:['string']};
  ...
  const response=await fetch(`${misoApi}/chat/completions`,{method:'POST',headers:{authorization:`Bearer ${misoKey}`,'content-type':'application/json'},body:JSON.stringify({model:'miso',temperature:0.55,messages:[{role:'system',content:'You create safe, coherent, practical, genuinely distinct recipes for Shua. Return ONLY one JSON object matching the provided schema. NEVER repeat or lightly rename any meal in avoidRecipeTitles; choose a different dish, cooking method, flavor profile, and composition. Treat allergy and exclusion text as absolute: never use peanuts, tree nuts, nut flours, nut oils, or nut-derived pesto, and never use pork. Prefer chicken breast and the supplied taste profile. Every recipe must be a complete, nicely plated meal pairing the main protein with rice, pasta, potatoes, and/or cooked vegetables—not protein alone. Default to exactly two servings unless the request explicitly says otherwise: dinner now and lunch tomorrow. Provide realistic integer calorie and protein estimates per serving. Prefer current inventory, use canonical ingredient UUIDs exactly when names match, include explicit food-safety temperatures where relevant, and never invent an image URL.'},{role:'user',content:JSON.stringify(prompt)}]})});
...
async function runOnce(){
  ...
  if(recipeJob){try{const [catalog,inventory]=await Promise.all([shua<Ingredient[]>('/ingredients'),shua<InventoryRow[]>('/inventory')]);let completed:GeneratedRecipe|undefined;for(let attempt=0;attempt<3&&!completed;attempt++){const recipe=await askMiso(recipeJob,catalog,inventory);try{await shua(`/recipe-jobs/${recipeJob.id}/complete`,'POST',recipe);completed=recipe}catch(error){const message=error instanceof Error?error.message:'';const retryable=['already created','allergy','pork exclusion','wrong serving count','pair the protein','did not pass validation'].some(reason=>message.includes(reason));if(!retryable||attempt===2)throw error;recipeJob.request.avoidRecipeTitles=[...(recipeJob.request.avoidRecipeTitles??[]),recipe.title]}}process.stdout.write(`Completed recipe job ${recipeJob.id}: ${completed?.title}\n`)}catch(error){const code=error instanceof Error?error.message:'miso_worker_failed';process.stderr.write(`Recipe job ${recipeJob.id} failed: ${code}\n`);await shua(`/recipe-jobs/${recipeJob.id}/fail`,'POST',{errorCode:code}).catch(()=>undefined)}return true}
