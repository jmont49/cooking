...
export function PlanWeek(){
  ...
  const picks=useMemo(()=>s.recommendations.filter(item=>item.recipe.id!==meal?.recipeId).slice(0,6),[s.recommendations,meal]);
  ...
  return <>
    <PageTitle eyebrow="Sequential week planner" title="Plan one good day at a time." description="Each choice reserves its ingredients and reshapes the recommendations, projected inventory, budget, and grocery list for the days that follow." action={<div className="flex items-center gap-2"><button disabled={weekOffset===0} onClick={()=>showWeek(0)} className="btn-secondary disabled:cursor-not-allowed disabled:opacity-35" aria-label="Show current week"><ChevronLeft size={17}/></button><span className="min-w-36 text-center text-sm font-semibold"><span className="block text-xs uppercase tracking-wider text-ink/40">{weekOffset===0?'Current week':'Next week'}</span>{weekLabel(dates)}</span><button disabled={weekOffset===1} onClick={()=>showWeek(1)} className="btn-secondary disabled:cursor-not-allowed disabled:opacity-35" aria-label="Show next week"><ChevronRight size={17}/></button></div>}/>
    ...
    <div className="mt-9 flex items-end justify-between"><div><p className="eyebrow"><Sparkles className="mr-1 inline" size={13}/>Recalculated now</p><h2 className="mt-1 text-3xl">Best for {new Date(selected+'T12:00').toLocaleDateString(undefined,{weekday:'long'})}</h2></div><div className="hidden text-right text-xs text-ink/45 sm:block">{s.inventory.reduce((count,item)=>count+(Number(item.reserved)>0?1:0),0)} ingredients currently reserved<br/>{s.grocery.length} grocery items projected</div></div>
