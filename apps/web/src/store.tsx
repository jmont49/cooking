...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const clearRecipes=async()=>{setRecipesError('');try{if(!demoMode)await recipeApi.deleteAll();setState(s=>({...s,recipes:[],meals:s.meals.filter(meal=>!meal.recipeId)}))}catch(error){const message=error instanceof Error?error.message:'The recipe collection could not be cleared.';setRecipesError(message);throw error}};
